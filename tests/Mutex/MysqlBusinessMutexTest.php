...
class MysqlBusinessMutexTest extends TestCase
{
    ...
    {
        $expired = 1;
        $operationName = 'phpunit.MysqlBusinessMutexTest.20200424';
        $id = 1;
        $obj = new BusinessMutex($this->getLockRepository(), $expired, $operationName, $id);
        $this->assertEquals('phpunit.MysqlBusinessMutexTest.20200424:1', $obj->getName());
    }
    ...
    {
        $expired = Helper::secondsToMilliseconds(3);
        $operationName = 'phpunit.MysqlBusinessMutexTest.20200424';
        $id = 3;
        $obj = new BusinessMutex($this->getLockRepository(), $expired, $operationName, $id);
        $obj->lock();
